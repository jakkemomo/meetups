...
@pytest.mark.asyncio
async def test_chats_list_valid_from_sender(
    async_client,
    async_user,
    async_user_2,
    event,
    chat_event_add_user,
    chat_event_add_user_2,
    chat_direct_user_user_2,
    chat_event_add_message,
    chat_direct_add_message,
):
    # User log in
    ...
    assert results[1]["last_message_text"] == chat_direct_add_message.message_text
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_chats_list_valid_from_receiver(
    async_client,
    async_user,
    async_user_2,
    event,
    ...
    assert results[0]["type"] == Chat.Type.EVENT
    assert results[0]["last_message_text"] == chat_event_add_message.message_text
    assert results[0]["last_message_is_owner"] is False
    assert results[0]["last_message_created_at"] is not None
    assert results[0]["unread_message_counter"] == 1

    assert results[1]["id"] == chat_direct_user_user_2.id
    assert results[1]["name"] == async_user.username
    assert results[1]["image_url"] == async_user.image_url
    assert results[1]["type"] == Chat.Type.DIRECT
    assert results[1]["last_message_text"] == chat_direct_add_message.message_text
    assert results[1]["last_message_is_owner"] is False
    assert results[1]["last_message_created_at"] is not None
    assert results[1]["unread_message_counter"] == 1

...
async def test_send_message_valid(
    application, async_client, async_user, async_user_2, event_chat_with_users
):
    # user log_in
    ...
    # user connects to ws
    message_communicator = get_chat_communicator(application, async_user, event_chat_with_users)
    ...
    message_communicator_2 = get_chat_communicator(
        application, async_user_2, event_chat_with_users
    )
    connected, subprotocol = await message_communicator_2.connect()
    assert connected

    notification_communicator_2 = get_communicator(application, async_user_2)
    ...
    await async_client.post(
        reverse(CHATS_SEND_MESSAGE_URL, args=[event_chat_with_users.id]), headers=header, data=data
    )
    ...
    message_object = await database_sync_to_async(Message.objects.filter)(
        created_by=async_user, chat=event_chat_with_users, message_text=data.get("message_text")
    )
    ...
    await asyncio.gather(message_communicator.disconnect(), message_communicator_2.disconnect())
...
async def test_send_message_twice_valid(
    ...
    # user connects to ws
    message_communicator_1 = get_chat_communicator(application, async_user, event_chat_with_users)
    connected, subprotocol = await message_communicator_1.connect()
    assert connected

    notification_communicator_1 = get_communicator(application, async_user)
    ...
    message_object = await database_sync_to_async(Message.objects.filter)(
        created_by=async_user, chat=event_chat_with_users, message_text=data_1.get("message_text")
    )
    message_object = await database_sync_to_async(message_object.first)()
    assert message_object
    ...
    await asyncio.gather(message_communicator_1.disconnect(), message_communicator_2.disconnect())


@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_send_message_chat_not_found(application, async_client, async_user):
    # user connects to ws
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_send_message_not_participant(application, async_client, async_user, event):
    # user connects to ws
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_send_message_too_long(application, async_client, async_user, event_chat_with_users):
    # user log_in
    token = await async_get_tokens(async_user)
    header = {"Authorization": "Bearer " + token}

    # user connects to ws
    communicator = get_chat_communicator(application, async_user, event_chat_with_users)
    connected, subprotocol = await communicator.connect()
    assert connected
...
async def test_concurrent_connections(
    ...
    response_ws, response_ws_2 = await asyncio.gather(
        message_communicator_1.receive_json_from(), message_communicator_2.receive_json_from()
    )
    assert response_ws.get("type") == response_ws_2.get("type") == "chat_message"
    assert (
        response_ws.get("data")
        == response_ws_2.get("data")
        == {
            "to_chat_id": event_chat_with_users.id,
            "from_user_id": async_user.id,
            "message_text": data_1.get("message_text"),

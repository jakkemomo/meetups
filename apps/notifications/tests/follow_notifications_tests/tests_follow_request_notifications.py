...
@pytest.mark.asyncio
async def test_follow_request_valid(application, async_client, async_user, async_user_2_private):
    ...
    response = await async_client.post(
        reverse(FOLLOW_URL, args=[async_user_2_private.id]), headers=header
    )
    ...
    assert response_ws.get("data") == {
        ...
        "additional_data": {"follower_status": Follower.Status.PENDING},
    }
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_follow_request_unauthorized(application, async_client, async_user_2_private):
    # someone follows user_2
    response = await async_client.post(reverse(FOLLOW_URL, args=[async_user_2_private.id]))
...
async def test_follow_request_valid_base(
    application, async_client, async_user, async_user_2_private
):

...
@pytest.mark.asyncio
async def test_follow_valid(application, async_client, async_user, async_user_2):
    ...
    response = await async_client.post(reverse(FOLLOW_URL, args=[async_user_2.id]), headers=header)
    ...
    assert response_ws.get("data") == {
        ...
        "additional_data": {"follower_status": Follower.Status.ACCEPTED},
    }

    notification_object = await database_sync_to_async(Notification.objects.filter)(
        created_by=async_user, recipient=async_user_2, type=Notification.Type.NEW_FOLLOWER
    )
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_follow_unauthorized(application, async_client, async_user_2):
    # someone follows user_2
    response = await async_client.post(reverse(FOLLOW_URL, args=[async_user_2.id]))
    assert response.status_code == 401


@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_follow_user_not_found(application, async_client, async_user):
    # user logs in and follows someone
    token = await async_get_tokens(async_user)
    header = {"Authorization": "Bearer " + token}
    response = await async_client.post(reverse(FOLLOW_URL, args=[1000]), headers=header)

...
async def test_accept_valid(
    ...
    response = await async_client.post(
        reverse(ACCEPT_URL, args=[async_user_private.id]), headers=header
    )
...
async def test_accept_unauthorized(
    application, async_client, async_user_private, async_follower_user_pending_private
):
    # someone accepts user's request
    response = await async_client.post(reverse(ACCEPT_URL, args=[async_user_private.id]))
...
async def test_accept_request_not_found(
    application, async_client, async_user_private, async_user_2_private
):
    # user connects to ws
    communicator = get_communicator(application, async_user_private)
    connected, subprotocol = await communicator.connect()
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio
async def test_accept_user_not_found(application, async_client, async_user):
    # user logs in and accepts someone
    token = await async_get_tokens(async_user)
    header = {"Authorization": "Bearer " + token}
    response = await async_client.post(reverse(ACCEPT_URL, args=[1000]), headers=header)

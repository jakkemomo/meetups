...
def test_email_notifications_preferences_patch_valid(
        api_client,
        user,
):
    # user logs in
    ...
    response = api_client.patch(
        reverse(EMAIL_PREFERENCES_GET_URL),
        data=data
    )
...
@pytest.mark.django_db
def test_email_notifications_preferences_patch_unauthorized(
        api_client,
        user,
):
    # user gets his notifications preferences
...
@pytest.mark.django_db
def test_email_notifications_preferences_patch_invalid_data(
        api_client,
        user,
):
    # user logs in

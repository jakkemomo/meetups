...
def test_in_app_notifications_preferences_put_valid(api_client, user):
    # user logs in
...
@pytest.mark.django_db
def test_in_app_notifications_preferences_put_unauthorized(api_client, user):
    # user gets his notifications preferences
    data = {
        "system": False,
        "new_message": False,
...
@pytest.mark.django_db
def test_in_app_notifications_preferences_put_invalid_data(api_client, user):
    # user logs in
    token = get_tokens(user)
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)

...
class Command(BaseCommand):
    ...
    def update_event_state(self, event):
        if not event.repeatable and self.is_finished(event):
            ...
            event.save()
            self.stdout.write(
                self.style.SUCCESS('Successfully updated event "%s"' % event.id)
            )
    ...
    def is_finished(event):
        ...
        if event.end_date and event.end_time:
            end_datetime = datetime.datetime.combine(event.end_date, event.end_time, tzinfo=datetime.timezone.utc)
        elif event.end_date and not event.end_time:
            end_datetime = datetime.datetime.combine(event.end_date, datetime.time(23, 59, 59), tzinfo=datetime.timezone.utc)
        elif not event.end_date and event.end_time:
            end_datetime = datetime.datetime.combine(event.start_date, event.end_time, tzinfo=datetime.timezone.utc)
        else:
            end_datetime = datetime.datetime.combine(event.start_date, datetime.time(23, 59, 59), tzinfo=datetime.timezone.utc)
        if now > end_datetime:
            return True
        return False

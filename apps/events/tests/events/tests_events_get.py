...
def test_event_get_unauthorised_valid(api_client, event):
    response = api_client.get(reverse(EVENTS_GET_URL, args=[event.id]))
    ...
    assert response.data == {
        ...
        "average_rating": (
            sum(event.ratings.values_list("value", flat=True)) / len(event.ratings.all())
            if len(event.ratings.all())
            else 0.0
        ),
        ...
        "city": {
            ...
            "longitude": (
                str(event.city.longitude) if event.city.longitude else event.city.longitude
            ),
...
@pytest.mark.django_db
def test_event_get_authorised_valid(api_client, event, user_2, event_user_2_is_participant):

...
def test_event_update_valid(api_client, user_2, event_created_by_user_2, event_data):
...
@pytest.mark.django_db
def test_event_update_repeatable_valid(api_client, user_2, event_created_by_user_2, event_data):
...
@pytest.mark.django_db
def test_event_update_free_valid(api_client, user_2, event_created_by_user_2, event_data):
...
@pytest.mark.django_db
def test_event_update_cost_no_currency(api_client, user_2, event_created_by_user_2, event_data):
...
@pytest.mark.django_db
def test_event_update_unauthenticated(api_client, user_2, event_created_by_user_2, event_data):
    response = api_client.put(
        reverse(EVENTS_GET_URL, args=[event_created_by_user_2.id]), data=event_data, format="json"
    )
...
@pytest.mark.django_db
def test_event_update_not_verified(api_client, user_2, event_created_by_user_2, event_data):
    # unverified email
...
@pytest.mark.django_db
def test_event_update_not_creator(api_client, user, user_2, event_created_by_user_2, event_data):
...
@pytest.mark.django_db
def test_event_update_participant_not_creator(api_client, user, event, event_data):
    token = get_tokens(user)
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)
    response = api_client.put(

...
class EventViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        event_instance = self.get_object()
        if event_instance.image_url != request.data.get("image_url", event_instance.image_url):
            delete_image_if_exists(event_instance)
    ...
    def register_for_event(self, request, event_id: int):
        ...
        if (
            event.type == "private"
            and request.COOKIES.get("private_event_key") != event.private_token
        ):
    ...
    @swagger_auto_schema(request_body=no_body, tags=["events"])
    @action(
        methods=["post"],
        detail=True,
        permission_classes=[EventPermissions],

...
def test_list_followers_accepted(
    ...
    response = api_client.get(
        reverse(LIST_FOLLOWERS_URL, args=[mock_test_user_2.id])
    )
    assert response.status_code == 200
    assert response.data[0].get("id") is not None
    assert response.data[0].get("user") == mock_test_user_2.id
    assert response.data[0].get("follower") == mock_test_user.id
...
def test_list_followers_accepted_private(
    ...
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)
    response = api_client.get(
        reverse(LIST_FOLLOWERS_URL, args=[mock_test_user_2_private.id])
    )
    assert response.status_code == 200
    assert response.data[0].get("id") is not None
    assert response.data[0].get("user") == mock_test_user_2_private.id
    assert response.data[0].get("follower") == mock_test_user_private.id
    assert response.data[0].get("status") == Follower.Status.ACCEPTED
...
def test_list_followers_no_creds(
    ...
    response = api_client.get(
        reverse(LIST_FOLLOWERS_URL, args=[mock_test_user.id])
    )

...
def test_accept_pending_private(api_client, user_private, user_2_private):
    token = get_tokens(user_2_private)
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)
    response = api_client.post(reverse(ACCEPT_URL, args=[user_private.id]))
...
@pytest.mark.django_db
def test_accept_declined_private(api_client, user_private, user_2_private):
    token = get_tokens(user_2_private)
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)
    response = api_client.post(reverse(ACCEPT_URL, args=[user_private.id]))
    assert response.status_code == 200
...
@pytest.mark.django_db
def test_accept_current_user(api_client, user):
    token = get_tokens(user)
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)
    response = api_client.post(reverse(ACCEPT_URL, args=[user.id]))
    assert response.status_code == 403
    assert response.data == {"detail": ErrorDetail(string="This is a current user", code="error")}


@pytest.mark.django_db
def test_accept_user_not_found(api_client, user):
    token = get_tokens(user)
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)
    response = api_client.post(reverse(ACCEPT_URL, args=[100]))
    assert response.status_code == 404
    assert response.data == {"detail": ErrorDetail(string="User not found", code="error")}


@pytest.mark.django_db
def test_accept_no_creds(api_client, user):
    response = api_client.post(reverse(ACCEPT_URL, args=[user.id]))
...
@pytest.mark.django_db
def test_accept_accepted(api_client, user, user_2):
    token = get_tokens(user_2)
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)
    response = api_client.post(reverse(ACCEPT_URL, args=[user.id]))
    assert response.status_code == 409
    assert response.data == {"detail": f"{user} is already following you"}


@pytest.mark.django_db
def test_accept_follower_not_found(api_client, user, user_2):
    token = get_tokens(user_2)
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)
    response = api_client.post(reverse(ACCEPT_URL, args=[user.id]))
    assert response.status_code == 404

...
def test_patch_is_private_false_pending(api_client, user_2_private):
    data = {"is_private": False}
    token = get_tokens(user_2_private)
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)
    response = api_client.patch(path=reverse(PROFILE_URL, args=[user_2_private.id]), data=data)
...
@pytest.mark.django_db
def test_patch_is_private_false_declined(api_client, user_2_private):
    data = {"is_private": False}
    token = get_tokens(user_2_private)
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)
    response = api_client.patch(path=reverse(PROFILE_URL, args=[user_2_private.id]), data=data)
...
@pytest.mark.django_db
def test_patch_is_private_false_accepted(api_client, user_2_private):
    data = {"is_private": False}
    token = get_tokens(user_2_private)
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)
    response = api_client.patch(path=reverse(PROFILE_URL, args=[user_2_private.id]), data=data)
...
@pytest.mark.django_db
def test_patch_is_private_true_accepted(api_client, user_2):
    data = {"is_private": True}
    token = get_tokens(user_2)
    api_client.credentials(HTTP_AUTHORIZATION="Bearer " + token)
    response = api_client.patch(path=reverse(PROFILE_URL, args=[user_2.id]), data=data)

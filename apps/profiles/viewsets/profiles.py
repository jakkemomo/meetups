...
from apps.events.filters import EventFilter, TrigramSimilaritySearchFilter
...
class MyProfileViewSet(generics.RetrieveAPIView):
    ...
    @swagger_auto_schema(responses={status.HTTP_200_OK: ProfileRetrieveSerializer}, tags=["users"])
...
class ProfileEventViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticatedOrReadOnly, IsAuthenticated, FollowerPermissions]
    ...
    def get_queryset(self):
        ...
        if self.action == "list_user_finished_events" and user_id:
            user = get_user_object(user_id)
            self.check_object_permissions(self.request, user)
            self.queryset = self.queryset.filter(end_date__lt=timezone.now())
        ...
        if self.action == "list_user_planned_events" and user_id:
            user = get_user_object(user_id)
            self.check_object_permissions(self.request, user)
            self.queryset = self.queryset.filter(start_date__gt=timezone.now())
        ...
        self.queryset = (
            self.queryset.prefetch_related("category", "tags", "city")
            .annotate(
